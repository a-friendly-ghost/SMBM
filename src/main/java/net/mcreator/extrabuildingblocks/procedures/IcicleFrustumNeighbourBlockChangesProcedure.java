...
public class IcicleFrustumNeighbourBlockChangesProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if ((world.getBlockState(BlockPos.containing(x, y - 1, z))).getBlock() == ExtraBuildingBlocksModBlocks.ICICLE_FRUSTUM.get()) {
			if ((world.getBlockState(BlockPos.containing(x, y + 1, z))).getBlock() == ExtraBuildingBlocksModBlocks.ICICLE_MIDDLE.get()
					|| (world.getBlockState(BlockPos.containing(x, y + 1, z))).getBlock() == ExtraBuildingBlocksModBlocks.ICICLE_BASE.get()) {
				world.setBlock(BlockPos.containing(x, y, z), ExtraBuildingBlocksModBlocks.ICICLE_MIDDLE.get().defaultBlockState(), 3);
			} else {
